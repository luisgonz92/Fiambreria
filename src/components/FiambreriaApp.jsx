...
export default function FiambreriaApp() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [cats, arts, sls, prs] = await Promise.all([
        db.getCategories(),
        db.getArticles(),
        db.getSales(),
        db.getPurchases(),
  ...
  const refresh = useCallback(async () => {
    const [arts, sls, prs] = await Promise.all([db.getArticles(), db.getSales(), db.getPurchases()]);
    setArticles(arts); setSales(sls); setPurchases(prs);
  }, []);
  ...
  return (
    <div className="app">
      ...
      <main className="main">
        ...
        <div className="page">
          {pg === 'dashboard' && <Dashboard articles={articles} sales={sales} purchases={purchases} setPg={setPg} lowStock={lowStock} outStock={outStock} />}
          ...
          {pg === 'reports' && <ReportsPage articles={articles} sales={sales} purchases={purchases} />}
        </div>
      </main>
...
// ============ DASHBOARD ============
function Dashboard({ articles, sales, purchases, setPg, lowStock, outStock }) {
  ...
  return (
    <div>
      ...
      <div className="kpi-g">
        ...
        <div className="kpi kpi-r"><div className="kpi-l">Compras Totales</div><div className="kpi-val" style={{ color: 'var(--rd)' }}>{money(allExpense)}</div><div className="kpi-s">{purchases.length} facturas</div></div>
      </div>
...
// ============ REPORTS ============
function ReportsPage({ articles, sales, purchases }) {
